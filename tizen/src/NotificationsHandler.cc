...
namespace btu{
    ...
    auto NotificationsHandler::notifyUIThread(const std::string& method, const google::protobuf::MessageLite& mess) noexcept -> void{
        std::vector<uint8_t> encodable(mess.ByteSizeLong());
        mess.SerializeToArray(encodable.data(), mess.ByteSizeLong());
        _methodChannel->InvokeMethod(method, std::make_unique<flutter::EncodableValue>(encodable));
    }
}
